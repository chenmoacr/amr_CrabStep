...
# ----- Repo-relative paths (Crab Step open-source release) -----
# All paths resolve relative to the repository root (one level up from `core/`).
# The host model is supplied via the GEMMA_PATH environment variable.
REPO_ROOT = Path(__file__).resolve().parent.parent
MODEL_PATH = os.environ.get("GEMMA_PATH",
                            str(REPO_ROOT.parent / "models" / "gemma-4-E2B-it"))
DATA_DIR = REPO_ROOT / "data"
NEURONS_JSON = REPO_ROOT / "core" / "neurons.json"
DEVICE = os.environ.get("CRABSTEP_DEVICE", "cuda:0")
...
def build_suppression_set(cfg):
    ...
    for src in sup_cfg.get("add_recruits", []) or []:
        ...
        sp = Path(src["source"])
        path = sp if sp.is_absolute() else (REPO_ROOT / sp)
        if not path.exists():
